...
def draw_stadium_lights():
    ...
    for x, y, z, rx, rz in positions:
        ...
        # Front
        glVertex3f(x - pole_width, y, z)
        glVertex3f(x + pole_width, y, z)
        glVertex3f(x + pole_width, y + pole_height, z)
        glVertex3f(x - pole_width, y + pole_height, z)

        # Back
        glVertex3f(x - pole_width, y, z + pole_width)
        glVertex3f(x + pole_width, y, z + pole_width)
        glVertex3f(x + pole_width, y + pole_height, z + pole_width)
        glVertex3f(x - pole_width, y + pole_height, z + pole_width)
        ...
        # ================= LIGHT HEAD (BIG SQUARE QUAD) =================
# ================= LIGHT HEAD (BIG SQUARE QUAD) =================
        glPushMatrix()

        head_offset_y = 2.6   # <<< raises the light head

        glTranslatef(x, y + pole_height + head_offset_y, z)
...
def draw_hud():
    ...
    glColor3f(1.0, 1.0, 1.0)
    glRasterPos2f(sx, sy)
    for ch in f"SCORES (first to {WIN_SCORE}):":
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, ord(ch))
    ...
    if game.game_over:
        ...
        winner = game.winner_team.upper() if game.winner_team else "N/A"
        score_text = f"WINNER: {winner}  (First to {WIN_SCORE})"
        for ch in score_text:
            glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, ord(ch))
...
def keyboard(key, x, y):
    ...
    if current_state == MENU:
        ...
        elif (key_char == 'w' or key_char == 'W') and current_state == PLAYING and ball_owner == ("human", None):
            push_ai_forward_backward(direction=1)

        elif (key_char == 's' or key_char == 'S') and current_state == PLAYING and ball_owner == ("human", None):
            push_ai_forward_backward(direction=-1)
