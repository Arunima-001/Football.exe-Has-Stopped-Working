...
difficulty_selection = 0  #ARU easy(0), hard(1)
game_over_selection = 0  # ARU: restart(0), quit(1)
...
ai_players = [
    ...
    {"pos": [ 0,0,-6],"rot":0, "speed": 0.01, "name":"AI-3", "team":"blue"},
]
...
#ARU Superpower - random spawn after each goal
superpower = {
    'active': False,         
    'type': None,            #freeze/speed
    'pos': [0, 0, 0],       
    'spawn_time': 0,         #when it popped up
    'lifetime': 3.0,         #3 seconds
    'effect_active': False,  
    'effect_time': 0,        #effect start
    'effect_duration': 5.0,  #5 seconds
    'rotation': 0,           #ghure effect
...
def spawn_superpower(): #ARU:Randomly spawn
    ...
    #possibility of spawn (90%)
    if random.random() < 0.90:
        superpower['active'] = True
        
        #toggle freeze and speed
...
def check_goalkeeper_collision():    
    ...
    EXTRA_D = 0.5
    EXTRA_H = 0.25    
    for side, goalie in goalies.items():    #side=north/east/south/west
...
def draw_difficulty_selection(): #ARU difficulty selection
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
...
def update_ai_players():
    ...
    while i < len(ai_players):
        ...
        separate_ai(i, min_sep=1.1, push=0.04)    #keep distance from other
...
def display():
    if current_state == MENU:
        draw_menu()
    elif current_state == DIFFICULTY_SELECTION:  #ARU:Show difficulty selection
        ...
        for i, ai in enumerate(ai_players):
            draw_ai_player(ai, i)
    elif current_state == GAME_OVER: #ARU: Game over show stats
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
...
def keyboard(key, x, y):
    ...
    #ARU: Handle difficulty screen
        ...
        if current_state == GAME_OVER:
            if key_char == '\r':  # ENTER key
                if game_over_selection == 0:  # ARU:Restart w same settings
...
def special_keys(key, x, y):
    ...
    #ARU:choose difficulty  
        ...
        if key == GLUT_KEY_UP:
            ...
            if current_state == GAME_OVER:  #ARU:navigate game over button
